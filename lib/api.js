...
export class RealtimeAPI extends RealtimeEventHandler {
  ...
  async connect() {
    ...
    if (isBrowserRuntime) {
      ...
      // Bun's ws compatibility can drop headers when called with
      // (url, [], options). Use the 2-arg form when no subprotocols are set.
  ...
  receive(eventName, event) {
    // Filter out high-frequency audio events from logging
    ...
    // Always add "server." prefix to maintain consistent event naming
