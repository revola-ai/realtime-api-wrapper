 ...
 * @property {"text"|"output_text"} type
 * @property {string} text
 */

/**
 * @typedef {Object} AudioContentType
 * @property {"audio"|"output_audio"} type
 * @property {string} [audio] base64-encoded audio data
 * @property {string|null} [transcript]
 */
...
export class RealtimeClient extends RealtimeEventHandler {
  ...
  _addAPIEventHandlers() {
    ...
    const handlerWithDispatch = (event, ...args) => {
      ...
      // DEBUG: Log conversation.updated dispatch for audio events
      if (event.type === 'response.audio.delta' && !this._convUpdateDebugLogged) {
        console.log('[RealtimeClient] 🐛 DEBUG: handlerWithDispatch for response.audio.delta');
        console.log('[RealtimeClient] 🐛 DEBUG: item:', !!item);
        console.log('[RealtimeClient] 🐛 DEBUG: delta:', delta ? Object.keys(delta) : null);
        console.log('[RealtimeClient] 🐛 DEBUG: Will dispatch conversation.updated:', !!item);
        this._convUpdateDebugLogged = true;
      }
      ...
      if (item) {
        ...
        // DEBUG: Log first conversation.updated dispatch with audio
        if (event.type === 'response.audio.delta' && !this._convDispatchLogged) {
          console.log('[RealtimeClient] ✅ DEBUG: Dispatching conversation.updated with audio delta');
          console.log('[RealtimeClient] ✅ DEBUG: item.id:', item.id);
          console.log('[RealtimeClient] ✅ DEBUG: delta.audio length:', delta?.audio?.length);
          this._convDispatchLogged = true;
        }
    ...
    this.realtime.on('server.response.output_item.added', handlerWithDispatch); // MUST dispatch to notify frontend of item creation
    ...
    // Support both beta (item.created) and GA (item.added/item.done) events
    this.realtime.on('server.conversation.item.created', (event) => {
      const { item } = handlerWithDispatch(event);
      if (!item) return; // Guard against null item (can happen when event fires before item.created)
    ...
    // Support both beta and GA event names for backward compatibility
    this.realtime.on(
...
/**
 * Normalizes session configuration to handle both legacy and modern formats
 * @private
 * @param {SessionResourceType} config
 * @returns {SessionResourceType}
 */
_normalizeSessionConfig(config) {
  // Create a copy to avoid mutating the original
  const normalized = { ...config };

  // Ensure audio formats are properly set
  if (normalized.input_audio_format === void 0) {
    normalized.input_audio_format = this.sessionConfig.input_audio_format || 'pcm16';
  }
  if (normalized.output_audio_format === void 0) {
    normalized.output_audio_format = this.sessionConfig.output_audio_format || 'pcm16';
  }
...
updateSession({
  ...
  // Normalize and prepare session config
  const session = this._normalizeSessionConfig(this.sessionConfig);
  session.tools = useTools;

  // Update conversation audio format tracking
  if (this.sessionConfig.output_audio_format) {
    this.conversation.setAudioFormat(this.sessionConfig.output_audio_format);
  }
