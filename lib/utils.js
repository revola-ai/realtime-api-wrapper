...
export class RealtimeUtils {
  ...
  /**
   * Gets the sample rate for a given audio format
   * @param {string} audioFormat - The audio format (pcm16, g711_ulaw, g711_alaw)
   * @returns {number} Sample rate in Hz
   */
  static getSampleRate(audioFormat) {
    switch (audioFormat) {
      case 'pcm16':
        return 24000; // 24 kHz
      case 'g711_ulaw':
      case 'g711_alaw':
        return 8000; // 8 kHz
      default:
        return 24000; // Default to pcm16
    }
  }

  /**
   * Gets bytes per sample for a given audio format
   * @param {string} audioFormat - The audio format (pcm16, g711_ulaw, g711_alaw)
   * @returns {number} Bytes per sample
   */
  static getBytesPerSample(audioFormat) {
    switch (audioFormat) {
      case 'pcm16':
        return 2; // 16-bit = 2 bytes
      case 'g711_ulaw':
      case 'g711_alaw':
        return 1; // 8-bit = 1 byte
      default:
   ...
   * @param {number} byteLength - Length of audio data in bytes
   * @param {string} audioFormat - The audio format
   * @returns {number} Duration in milliseconds
   */
  static calculateAudioDuration(byteLength, audioFormat) {
    const sampleRate = this.getSampleRate(audioFormat);
    const bytesPerSample = this.getBytesPerSample(audioFormat);
