...
export class RealtimeConversation {
  ...
  EventProcessors = {
    ...
    'response.output_item.added': (event) => {
      ...
      // Initialize tool object for function_call items
      if (newItem.type === 'function_call') {
        ...
        // Initialize arguments property if not present
        if (!newItem.arguments) {
          newItem.arguments = '';
        }
        // Check if there are queued arguments from earlier deltas
        if (this.queuedTranscriptItems[newItem.id] && this.queuedTranscriptItems[newItem.id].arguments) {
          newItem.formatted.tool.arguments = this.queuedTranscriptItems[newItem.id].arguments;
          newItem.arguments = this.queuedTranscriptItems[newItem.id].arguments;
          delete this.queuedTranscriptItems[newItem.id];
    ...
    'response.audio_transcript.delta': (event) => {
      ...
      if (item.content[content_index]) {
        if (item.content[content_index].transcript === undefined) {
    ...
    'response.audio.delta': (event) => {
      ...
      // DEBUG: Log first audio delta processing
      if (!this._audioDebugLogged) {
        console.log('[RealtimeConversation] 🐛 DEBUG: Processing response.audio.delta');
        console.log('[RealtimeConversation] 🐛 DEBUG: item_id:', item_id);
        console.log('[RealtimeConversation] 🐛 DEBUG: item found:', !!item);
        console.log('[RealtimeConversation] 🐛 DEBUG: itemLookup keys:', Object.keys(this.itemLookup));
        this._audioDebugLogged = true;
      }

      if (!item) {
        console.warn(`[RealtimeConversation] response.audio.delta: Item "${item_id}" not found. Queueing audio.`);
        if (!this.queuedSpeechItems[item_id]) {
      ...
      // DEBUG: Log successful audio processing
      if (!this._audioSuccessLogged) {
        console.log('[RealtimeConversation] ✅ DEBUG: Successfully processed audio delta, returning item and delta');
        console.log('[RealtimeConversation] ✅ DEBUG: appendValues length:', appendValues.length);
        this._audioSuccessLogged = true;
      }
    ...
    'response.function_call_arguments.delta': (event) => {
      ...
      // Ensure formatted.tool exists (safety check)
